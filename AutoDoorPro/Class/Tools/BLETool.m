...
+ (NSString *)hardwareInfo:(NSString *)info {
    return [NSString stringWithFormat:@"%@.%@",[[self replaceString:info] substringWithRange:NSMakeRange(6, 2)],[info substringWithRange:NSMakeRange(8, 2)]];
}

+ (NSString *)softwareInfo:(NSString *)info {
    return [NSString stringWithFormat:@"%@.%@-%@.%@",[[self replaceString:info] substringWithRange:NSMakeRange(10, 2)],[[self replaceString:info] substringWithRange:NSMakeRange(12, 2)],[[self replaceString:info] substringWithRange:NSMakeRange(14, 2)],[[self replaceString:info] substringWithRange:NSMakeRange(16, 2)]];
}

+ (NSInteger)speed:(NSString *)value {
    NSString *speed = [[self replaceString:value] substringWithRange:NSMakeRange(6, 2)];
    NSLog(@"--速度:%@",speed);
    return [NSString stringWithFormat:@"%ld",strtoul([speed UTF8String], 0, 16)].integerValue;
}

+ (NSInteger)damp:(NSString *)value {
    NSString *speed = [[self replaceString:value] substringWithRange:NSMakeRange(8, 2)];
    NSLog(@"--阻尼:%@",speed);
    return [NSString stringWithFormat:@"%ld",strtoul([speed UTF8String], 0, 16)].integerValue;
}
...
+ (NSInteger)openTime:(NSString *)value {
    ...
    NSString *speed = [[self replaceString:value] substringWithRange:NSMakeRange(10, 2)];
    NSLog(@"--开门:%@",speed);
    return [NSString stringWithFormat:@"%ld",strtoul([speed UTF8String], 0, 16)].integerValue;
}

+ (NSInteger)closure:(NSString *)value {
    NSString *speed = [[self replaceString:value] substringWithRange:NSMakeRange(14, 2)];
    NSLog(@"--紧闭力:%@",speed);
    return [NSString stringWithFormat:@"%ld",strtoul([speed UTF8String], 0, 16)].integerValue;
}
